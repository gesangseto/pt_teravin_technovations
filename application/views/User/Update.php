	...
	<div class="container">
		...
		<form class="form-horizontal" method="POST" action="<?= site_url('User/Create') ?>">
			...
			<div class="form-group">
				...
				<div class="col-sm-10">
					<input type="text" pattern="\d*" value="<?= @$user[0]['phone_number'] ?>" class="form-control" name="phone_number">
				</div>
			</div>
			<div class="form-group">
			...
			foreach ($user_address as $row) {
				...
				echo '<input type="text" value="' . $row['address'] . '" class="form-control"   maxlength="150jfgss" name="address" required>';
				echo '</div>';
				echo '</div>';
				$no = $no + 1;
			} ?>
